...
public class JoystickMovement : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    public JtkDirection joyStickdirection;
    ...
    public JtkDirection GetDirection(){
        if(HorizontalInput() == 0 && VerticalInput() == 0){
            joyStickdirection = JtkDirection.Center;
            return JtkDirection.Center;
        }
        else{
            if(Mathf.Abs(HorizontalInput()) > Mathf.Abs(VerticalInput())){
                if(HorizontalInput() > 0){
                    joyStickdirection = JtkDirection.Right;
                    return JtkDirection.Right;
                }
                else {
                    joyStickdirection = JtkDirection.Left;
                    return JtkDirection.Left;
                }
            }
            else{
                if(VerticalInput() > 0) {
                    joyStickdirection = JtkDirection.Up;
                    return JtkDirection.Up;
                }
                else {
                    joyStickdirection = JtkDirection.Down;
                    return JtkDirection.Down;
                }
            }
        }
    }
