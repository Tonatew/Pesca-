...
public class CableProceduralSimple : MonoBehaviour
{
	...
	[SerializeField] public float swayMultiplier = 1;
	[SerializeField] public float swayXMultiplier = 1;
	[SerializeField] public float swayYMultiplier = .5f;
	// How fast the cable will go back and forth per second.
	[SerializeField] float swayFrequency = 1;
	[SerializeField] float updateFrecuency;
	...
	void Update () 
	{
		if(nextUpdate < Time.time & endPointTransform){
			Start();
			Draw();
			nextUpdate = Time.time + updateFrecuency;
		}
	}
